...
export const CommentOfCommentApiContext = createContext();

export default function CommentOfCommentApiContextProvider({ children }) {
    ...
    async function fetchApiComment() {
        ...
        try {
            // console.log(postid)
            const response = await axios.post(`http://localhost:4000/api/v1/blog/commentOfComments/get/${postid}`,
    ...
    async function fetchApiCount() {
        ...
        try {
            ...
            const response = await axios.get(`http://localhost:4000/api/v1/blog/commentOfComments/count/${postid}`,
                {
                    headers: {
                        Authorization: `Bearer ${JSON.parse(localStorage.getItem("auth"))?.token}`
                    }
    ...
    return <CommentOfCommentApiContext.Provider value={value}>
        {children}
    </CommentOfCommentApiContext.Provider>
}